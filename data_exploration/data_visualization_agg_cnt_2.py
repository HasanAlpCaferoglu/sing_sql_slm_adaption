...
To run:
PYTHONPATH=src python -m data_exploration.data_visualization_agg_cnt_2
"""
...
def plot_syn_avg_vs_bird_dev(
    ...
    outfile_name: str = "agg_percentage_syn_avg_vs_bird_per_level.png",
) -> Path:
    """
    Plot a grouped bar chart comparing bird_dev vs the average of syn_* datasets
    (syn_train, syn_dev, syn_test) for `aggregation_cnt_per_level`.
    ...
    def get_agg_percentage_per_sql_per_level(section: Dict[str, Any]) -> Dict[str, float]:
        stats = section.get("aggregation_percentage_per_level")
        if not isinstance(stats, dict):
            raise ValueError("Missing 'aggregation_percentage_per_level' in a dataset section.")
        # Cast to float; ignore missing keys by returning 0.0
        return {lvl: float(stats.get(lvl, 0.0)) for lvl in levels}

    def get_total_agg_cnt_per_level(section: Dict[str, Any]) -> Dict[str, float]:
        stats = section.get("aggregation_cnt_per_level")
        if not isinstance(stats, dict):
            raise ValueError("Missing 'aggregation_cnt_per_level' in a dataset section.")
        # Cast to float; ignore missing keys by returning 0.0
        return {lvl: float(stats.get(lvl, 0.0)) for lvl in levels}
    ...
    bird_appspl_map: Dict[str, float] = get_agg_percentage_per_sql_per_level(data[bird_key])
    syn_appspl_maps: List[Dict[str, float]] = [get_agg_percentage_per_sql_per_level(data[k]) for k in syn_keys]
    syn_tacpl_maps: List[Dict[str, float]] = [get_total_agg_cnt_per_level(data[k]) for k in syn_keys]
    ...
    for lvl_idx, lvl in enumerate(levels):
        # vals = [m.get(lvl, 0.0) for m in syn_appspl_maps if lvl in m]
        # syn_avg.append(sum(vals) / len(vals) if vals else 0.0)
        total_vals = [m.get(lvl, 0.0) for m in syn_tacpl_maps if lvl in m]
        total_cnts = [m.get(lvl, 0.0) for m in syn_qcpl_maps if lvl in m]

        syn_avg.append(100 * sum(total_vals) / sum(total_cnts) if total_cnts else 0.0)

    bird_vals: List[float] = [bird_appspl_map[lvl]*100 for lvl in levels]
    ...
    plt.ylabel("Aggregation Percentage")
    plt.title("Aggregation Percentage for Each Level Across Datasets")
