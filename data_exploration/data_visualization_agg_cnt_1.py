...
"""
To run:
AGGREGATION COUNT SYNTHETIC DATA (TRAIN-DEV-TEST) VS BIRD DATA FOR SPECIFIC DATABASE

PYTHONPATH=src python -m data_exploration.data_visualization_agg_cnt_1
...
for key, label in datasets:
    ds = data.get(key, {})
    aggregation_stats = (ds or {}).get("aggregation_percentage_per_level", {})
    # Ensure order matches `levels`
    vals = [float(aggregation_stats.get(level, 0.0))*100 for level in levels]
...
out_path = data_exploration_dir_path / "agg_percentage_for_each_level_across_dataset.png"
