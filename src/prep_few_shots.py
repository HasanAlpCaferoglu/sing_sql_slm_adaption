...
from runner.prep_few_shots_runner import PrepFewShotsRunner
...
def prepare_few_shots(args):

    start_time = time.time()
    print("=== FEW-SHOT PREPERATION ===")
    prep_few_shot_runner = PrepFewShotsRunner(args, method="bm25")
    # prep_few_shot_runner.prep_eval_few_shots()
    prep_few_shot_runner.prep_synthetic_data_few_shots()

    duration = time.time() - start_time
    print(f"===== FEW-SHOT PREPARATION FINISHED IN {duration} seconds.")

if __name__ == "__main__":
    load_dotenv()
    args = parse_arguments()
    update_args(args)
    prepare_few_shots(args)
