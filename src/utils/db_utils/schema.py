...
class ColumnInfo:
    """
    Represents the metadata for a single column in a database table
    ...
    Attributes:
        ...
        column_meaning (str): Detail explanations about the column
        ...
        unique_values (List[str]): unique values
        value_statics (str):  statistics for values
    """
...
def get_primary_keys(table_schema: TableSchema) -> List[str]:
    ...
    """
    return [column_name for column_name, column_info in table_schema.columns.items() if column_info.primary_key]
...
class DatabaseSchema:
    ...
    def get_column_info(self, table_name: str, column_name: str) -> Optional[ColumnInfo]:
        ...
        if actual_table_name:
            actual_column_name = self.get_actual_column_name(actual_table_name, column_name)
            if actual_column_name:
                return self.tables[actual_table_name].columns[actual_column_name]
        return None
    ...
    def get_db_joinables_from_schema(self) -> Dict[str, List[str]]:
        """
        This function construct a dict storing each table's joinable tables.
        Note that this function only take advantage from the database schema's referential integrity. 
        (There might be missing referential integrities)
        """

        detail_schema_dict = self.to_dict(with_info=True)
        db_joinables = {t_name: [] for t_name in detail_schema_dict.keys()}

        for t_name, t_info_dict in detail_schema_dict.items():
            for c_name, c_info_dict in t_info_dict.items():
                foreign_keys = c_info_dict['foreign_keys']
                referenced_by = c_info_dict['referenced_by'] # List[List[str]] where first string represents the table name and the second string represents the table name
                # Capture the connections in foreign keys (foreign_keys)
                for connection in foreign_keys:
                    t_name_in_connection = connection[0]
                    if t_name_in_connection not in db_joinables[t_name]:
                        db_joinables[t_name].append(t_name_in_connection)
                    if t_name not in db_joinables[t_name_in_connection]:
                        db_joinables[t_name_in_connection].append(t_name)
                # Capture the connections in foreign keys (referenced_by)
                for connection in referenced_by:
                    t_name_in_connection = connection[0]
                    if t_name_in_connection not in db_joinables[t_name]:
                        db_joinables[t_name].append(t_name_in_connection)
                    if t_name not in db_joinables[t_name_in_connection]:
                        db_joinables[t_name_in_connection].append(t_name)
                # Capture the joinable tables on a column exist in another table
                for connection in foreign_keys:
                    fk_t_name = connection[0] 
                    fk_c_name = connection[1]
                    joinable_tables_on_third_table_column = self.get_table_joinables_on_third_table_column(
                        current_t_name=t_name,
                        fk_t_name=fk_t_name,
                        fk_c_name=fk_c_name
                    )
                    print(f"t_name: {t_name}")
                    print(f"joinable_tables_on_third_table_column: \n {joinable_tables_on_third_table_column}")
                    for joinable_t_name in joinable_tables_on_third_table_column:
                        if t_name not in db_joinables[joinable_t_name]:
                            db_joinables[joinable_t_name].append(t_name) 
                        if joinable_t_name not in db_joinables[t_name]:
                            db_joinables[t_name].append(joinable_t_name)

        return db_joinables
    
    def get_table_joinables_on_third_table_column(self, current_t_name:str, fk_t_name:str, fk_c_name:str) -> Dict[str, List[str]]:
        """"
        This function finds two joinable tables which can join on a column in another third table.
        Such join is possible as both tables have a column referrring to the column in another common table. 

        Args:
            current_t_name (str): Name of the table that is considered to find if it can joinable another table on a column existing in third table
            fk_t_name: str (str): Name of the table that one of the column of current table referring
            fk_c_name: str (str): Name of the column that one of the column of current table referring
        """

        detail_schema_dict = self.to_dict(with_info=True)
        joinable_tables_on_third_table_column: List[str] = []

        for t_name, t_info_dict in detail_schema_dict.items():
            if t_name == current_t_name:
                continue
            if t_name == fk_t_name:
                continue
            for c_name, c_info_dict in t_info_dict.items():
                foreign_keys = c_info_dict['foreign_keys']
                for connection in foreign_keys:
                    t_name_in_conneciton = connection[0]
                    c_name_in_connection = connection[1]
                    if t_name_in_conneciton == fk_t_name and fk_c_name == fk_c_name:
                        if t_name not in joinable_tables_on_third_table_column:
                            joinable_tables_on_third_table_column.append(t_name)

        return joinable_tables_on_third_table_column

                
    def get_tables_and_columns(self) -> Dict[str, List[str]]:
        """
        Get DatabaseSchema tables and columns in a dictionary 

        Returns:
            Dict[str, List[str]]: Keys are the names of tables and the values are the list of column names
        """

        return {table_name: list(table_info.columns.keys()) for table_name, table_info in self.tables.items()}

    
    def to_dict(self, with_info:Optional[bool]=False) -> Union[Dict[str, List[str]], Dict[str, Dict[str, Any]]]:
        """
        Converts the DatabaseSChema to a dictionary representation

        Return
            Union[Dict[str, List[str]], Dict[str, Dict[str, Any]]]: The dictionary representation of the schema
        """
        ...
        if not with_info:
            ...
            detailed_db_schema =  {
                table_name: {
                    column_name: {
                        "original_column_name": column_info.original_column_name,
                        "column_name": column_info.column_name,
                        "column_description": column_info.column_description,
                        "column_meaning": column_info.column_meaning,
                        ...
                        "value_statics": column_info.value_statics
                    }
                    for column_name, column_info in table_info.columns.items()
                }
                for table_name, table_info in self.tables.items()
