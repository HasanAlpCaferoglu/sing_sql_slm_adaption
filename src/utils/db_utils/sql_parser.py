...
def get_sql_columns_dict(db_path: str, sql: str) -> Dict[str, List[str]]:
    ...
    !! Note that retuned tables and column names are lowercased
    ...
    Returns:
        Dict[str, List[str]]: Dictionary of tables and their list of column names.
    """
    try:
...
def get_schema_items_as_set(schema_dict: Dict[str, List[str]], lowercase: Optional[bool]=False) -> Set[str]:
    """
...
def get_sql_schema_items_as_set(db_path:str, sql:str, lowercase: Optional[bool]=False) -> Set[str]:
    """"
    Extract the schema items in an SQL as a set of table_name.column_name string. 

    db_path (str): Path to the database file.
        sql (str): The SQL query string.
        
    Returns:
        Set[str]: Set of schema items.
    """

    sql_schema_dict = get_sql_columns_dict(db_path=db_path, sql=sql)
    sql_schema_items = get_schema_items_as_set(schema_dict=sql_schema_dict, lowercase=lowercase)

    return sql_schema_items
...
def get_filtered_schema_dict_from_similar_examples(db_path: Path, similar_examples: List[Dict[str, Any]]) -> Dict[str, List[str]]:
    ...
    """
    db_original_schema_dict=get_db_schema(db_path=db_path)
    ...
    for t2s_dict in similar_examples:
        ...
        for table, columns in sql_columns_dict.items():
            if table in db_original_schema_dict:
