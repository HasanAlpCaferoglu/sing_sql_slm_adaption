...
from utils.db_utils.db_values.preprocess_db_values import _create_minhash
...
def load_db_lsh(db_directory_path: str, prep_dir_name: str = "prep_schemaless") -> Tuple[MinHashLSH, Dict[str, Tuple[MinHash, str, str, str]]]:
    """
    Loads the LSH and MinHashes from the preprocesed files in the specified directory.
    ...
    try:
        with open(Path(db_directory_path) / prep_dir_name / f"{db_id}_lsh.pkl", "rb") as file:
            lsh = pickle.load(file)
        with open(Path(db_directory_path) / prep_dir_name/ f"{db_id}_minhashes.pkl", "rb") as file:
