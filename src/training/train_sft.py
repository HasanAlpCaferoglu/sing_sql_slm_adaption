...
def find_max_token_length_in_dataset(combined_dataset, accelerator=None):

    max_seq_len = 0
    for sample in tqdm(combined_dataset):
        sample_seq_len = sample["seq_len"]
        if sample_seq_len > max_seq_len:
            max_seq_len = sample_seq_len

    if accelerator:
        accelerator.print(f"+++++++++ MAX SEQUENCE LENGTH = {max_seq_len}")
    else:
...
def train_sft(args):
    ...
    hf_repo_name = f"{base_model_id_without_user}_{t2s_dataset_name[0]}{data_mode[0]}_{db_id_str}_{task_str}_e{epoch}_bs{bs}_gas{gas}_lr{learningrate}_fs{few_shot_cnt}{urifs}_cvd{use_cvd_init}"
    if use_reasoning:
        hf_repo_name = f"{hf_repo_name}_sftreason" # Reasoning with SFT

    hf_repo_name = f"{os.getenv('HF_USER')}/{hf_repo_name}"
    local_output_dir = Path(f"./{hf_repo_name}")  # this is where checkpoints will be saved
    local_output_dir.mkdir(parents=True, exist_ok=True)  # make sure directory exists
    ...
    accelerator.print(f"..Checking for local checkpoints under: {local_output_dir}")
    # checkpoint_paths = sorted(Path(local_output_dir).glob("checkpoint-*")) # lexical sorting (problem: ckpt-500 looks like ckpt-1000 )
    accelerator.print(f'Path(local_output_dir).glob("checkpoint-*"): {Path(local_output_dir).glob("checkpoint-*"),}')
    checkpoint_paths = sorted(
        Path(local_output_dir).glob("checkpoint-*"),
        key=extract_checkpoint_number
    )
    if checkpoint_paths:
        accelerator.print(f"Local checkpoints: {[str(ckpt_path_str) for ckpt_path_str in checkpoint_paths] }")
        resume_from_checkpoint = str(checkpoint_paths[-1])  # Use the latest one
        accelerator.print(f"✅ Found local checkpoint: {resume_from_checkpoint}. Resuming training.")
    else:
        accelerator.print("Proceeding to start training from scratch.")
    ...
    if bool(train_configs.get("use_lora", False)):
        accelerator.print("Setting PEFT configs...")
    ...
    accelerator.print(f"Model ({model.name_or_path}) is loaded.")
    ...
    find_max_token_length_in_dataset(combined_dataset, accelerator)
    ...
    accelerator.print("Starting Accelerate training loop...")
    ...
    if resume_from_checkpoint:
        try:
            ...
            accelerator.print(f"Resuming from step: {global_completed_steps}")
            accelerator.print(f"Resume epoch: {resume_epoch}, batch index: {resume_batch_idx}")
    ...
    for epoch in range(epoch):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            """
            # If you don't use zero_stage: 2. If you use zero_stage: 2 or 3 then sync is required but accelerator.accumulate use no_sync which cause error
            ...
            if accelerator.sync_gradients:
                ...
                if global_completed_steps % save_steps == 0 and accelerator.is_main_process:
                    ...
                    accelerator.print(f"Checkpoint saved at step {global_completed_steps} to {ckpt_path}")
                    ...
                    if len(checkpoints) > save_total_limit:
                        for ckpt_to_remove in checkpoints[:-save_total_limit]:
                            accelerator.print(f"Removing old checkpoint: {ckpt_to_remove}")
                            os.system(f"rm -rf {ckpt_to_remove}")

        accelerator.print("At the end of epoch")
